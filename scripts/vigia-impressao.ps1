...
# --- Vigilancia do BACKUP diario ---
# Desde 15/09/2026 o C:\EntregaCerta\backup-diario.ps1 busca toda manha, as 07:30,
# o backup do banco e as fotos do servidor. Backup que para de funcionar nao faz
# barulho nenhum: o sistema segue normal e a falta so aparece no dia em que se
# precisa dele. Por isso o vigia confere o arquivo que o backup so grava quando
# termina COMPLETO. Fica aqui em cima porque as checagens da impressao, logo
# abaixo, podem encerrar o script.
$ARQ_BACKUP_OK = Join-Path $PASTA 'backup\ultimo-backup-ok.txt'
$ESTADO_BACKUP = Join-Path $PASTA '.vigia-backup'
$HORAS_BACKUP  = 30   # rodada diaria + folga para um atraso sem virar alarme

function VigiarBackup {
  # So depois das 8h e com o PC acordado ha 45 min: o backup roda 07:30 e a
  # primeira carga das fotos pode demorar. Antes disso o "ultimo" e o de ontem
  # por definicao, e alarme falso diario ensina a ignorar o alarme.
  if ((Get-Date).Hour -lt 8 -or $acordadaHaMin -lt 45) { return }

  $ultimo = $null
  if (Test-Path $ARQ_BACKUP_OK) {
    $ultimo = try { [datetime]::Parse(((Get-Content $ARQ_BACKUP_OK -Raw).Split('|')[0]).Trim()) } catch { $null }
  }
  $jaAvisouBackup = if (Test-Path $ESTADO_BACKUP) { (Get-Content $ESTADO_BACKUP -Raw).Trim() } else { '' }

  if ($ultimo -and ((Get-Date) - $ultimo).TotalHours -lt $HORAS_BACKUP) {
    if ($jaAvisouBackup) {
      Anotar "OK - backup voltou a chegar (ultimo completo em $($ultimo.ToString('dd/MM HH:mm')))"
      Remove-Item $ESTADO_BACKUP -Force -ErrorAction SilentlyContinue
    }
    return
  }

  # Um aviso por dia basta: backup atrasado nao piora de 5 em 5 minutos.
  $hoje = (Get-Date).ToString('yyyy-MM-dd')
  if ($jaAvisouBackup -eq $hoje) { return }

  $quando = if ($ultimo) { "em $($ultimo.ToString('dd/MM HH:mm'))" } else { 'nunca' }
  Anotar "ALERTA - backup do EntregaCerta nao chegou ao PC; ultimo completo: $quando"
  Set-Content $ESTADO_BACKUP -Value $hoje -Encoding utf8
  Avisar 'EntregaCerta - backup nao chegou' ("O ultimo backup completo foi $quando.`n" +
    "O sistema continua funcionando normalmente.`n" +
    "Veja C:\EntregaCerta\backup\backup.log e avise o suporte.")
}
VigiarBackup
